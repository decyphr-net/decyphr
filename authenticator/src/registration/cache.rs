...
pub struct RegistrationCache<'a> {
    pub redis_pool: &'a Data<RedisPool>
}


impl<'a> RegistrationCache<'a> {
        ...
        let redis_con = self.redis_pool
            ...
            .map_err(
                |e| {
                    ...
                    return "We cannot activate your account at the moment";
                }
            )
    ...
    /// Set the user's registration token in Redis. This should also be set to expire
    /// based on the `ttl` config's `ttl` value
        ...
        match cmd("DEL")
            ...
            .query_async::<_, ()>(&mut redis_con).await {
                ...
                Err(e) => {
                    event!(target: "authenticator", Level::ERROR, "{}", e);
                    return Err(
                        "It appears that your activation token has expired"
                        .to_string()
                    )
                }
            };
