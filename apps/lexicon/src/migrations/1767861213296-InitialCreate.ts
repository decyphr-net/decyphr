...
export class InitialCreate1767861213296 implements MigrationInterface {
    name = 'InitialCreate1767861213296'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`statements\` (\`id\` int NOT NULL AUTO_INCREMENT, \`text\` varchar(2000) NOT NULL, \`meaning\` varchar(2000) NULL, \`language\` varchar(10) NOT NULL, \`source\` varchar(15) NOT NULL, \`clientId\` varchar(255) NOT NULL, \`fingerprint\` varchar(64) NOT NULL, \`createdAt\` timestamp NOT NULL, UNIQUE INDEX \`IDX_03773cd00eb2f5ce577b1a62b2\` (\`fingerprint\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`statement_tokens\` (\`id\` int NOT NULL AUTO_INCREMENT, \`position\` int NOT NULL, \`surface\` varchar(100) NOT NULL, \`lemma\` varchar(50) NOT NULL, \`pos\` varchar(20) NOT NULL, \`statementId\` int NULL, \`wordId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`statement_tokens\` ADD CONSTRAINT \`FK_e81f85ecff62b7edf3dc8b0d51c\` FOREIGN KEY (\`statementId\`) REFERENCES \`statements\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`statement_tokens\` ADD CONSTRAINT \`FK_5736bf6ed9bf05ae6d75a7d8ef3\` FOREIGN KEY (\`wordId\`) REFERENCES \`words\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`statement_tokens\` DROP FOREIGN KEY \`FK_5736bf6ed9bf05ae6d75a7d8ef3\``);
        await queryRunner.query(`ALTER TABLE \`statement_tokens\` DROP FOREIGN KEY \`FK_e81f85ecff62b7edf3dc8b0d51c\``);
        ...
        await queryRunner.query(`DROP TABLE \`statement_tokens\``);
        await queryRunner.query(`DROP INDEX \`IDX_03773cd00eb2f5ce577b1a62b2\` ON \`statements\``);
