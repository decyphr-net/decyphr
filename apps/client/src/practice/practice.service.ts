...
@Injectable()
export class PracticeGatewayService {
  private readonly practiceUrl =
    process.env.PRACTICE_SERVICE_URL || 'http://practice:3014';
  ...
  private async parseResponse(res: Response) {
    ...
    if (!res.ok) {
      const body = contentType.includes('application/json')
        ...
        : await res.text();
      throw new Error(`Practice service error (${res.status}): ${body}`);
    }
  ...
  private buildUrl(
    ...
    const url = new URL(`${this.practiceUrl}${path}`);
  ...
  async get(path: string, clientId: string, query?: Record<string, string | undefined>) {
    ...
    try {
      ...
      throw new Error(
        `Practice service unreachable: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
